...
const getScoreBoard = async function (req, res) {
  const top10List = await rankProvider.getTop10List(); //FIXME: async try-catch
...
const getMyRank = async function (req, res) {
  ...
  const doesNicknameExistForRankingScore = await rankProvider.checkNicknameForRankingScore(nickname); //FIXME: async try-catch
