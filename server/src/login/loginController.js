...
const login = async function (req, res) {
  ...
  // validation
  if (!nickname0) return res.json(errResponse(Message.EMPTY_NICKNAME));
  if (!password0) return res.json(errResponse(Message.EMPTY_PASSWORD));
  if (nickname0.length < 4 || nickname0.length > 8) return res.json(errResponse(Message.LENGTH_NICKNAME));
  if (password0.length < 4 || password0.length > 10) return res.json(errResponse(Message.LENGTH_PASSWORD));
  if (!(score0 > 0)) return res.json(errResponse(Message.NAN_SCORE));
  ...
  try {
    ...
    if (!doesNicknameExistForNo) {
      ...
      return res.json(response(Message.SUCCESS_CREATE, { insertId: createRecordResult }));
    }
    ...
    if (!doesPasswordCorrectForScore) return res.json(errResponse(Message.NOT_MATCHED_PASSWORD));
    ...
    if (score0 > dbScore) {
      ...
      return res.json(response(Message.SUCCESS_UPDATE, { savedScore: dbScore, newScore: score0 }));
    }
    return res.json(response(Message.SUCCESS_LOW_SCORE, { savedScore: dbScore }));
